 ...
 */
public class FetchRandomData extends AsyncTask<String, Void, Void> {
    private final String LOG_CAT = getClass().getSimpleName();
    private Context context;
    public FetchRandomData(Context context) {
    ...
    protected Void doInBackground(String... params) {
        ...
        if (jsonStr!=null){
            ...
            for (int i=0;i< 5;i++) {
                String datail = dt[i];//getResponce("wallpaper_info",ids[i]);
                if (datail!=null){
                    ...
                    if (d.length>0){
                        ...
                        wall.put(DatabaseContract.WallpaperEntries.COLUMN_METHORD,params[0]);
                        cVVector.add(wall);
                    }
                }
            }
        ...
        try {
            if (cVVector.size() > 0) {
                ...
                context.getContentResolver().bulkInsert(DatabaseContract.WallpaperEntries.CONTENT_URI, cvArray);

            }
        } catch (Exception e){
            e.printStackTrace();
        }
        return null;//getWallpaperDataFromJson(forecastJsonStr);
    }
